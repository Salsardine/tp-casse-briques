...
@export var nb_colonnes = 2
@export var nb_lignes = 1
@export var espacement_x = 20
@export var espacement_y = 15
...
func generer_briques() -> void:
	...
	var largeur_totale = nb_colonnes * (taille_brique.x + espacement_x) - espacement_x
	...
	for ligne in range(nb_lignes):
		for colonne in range(nb_colonnes):
			...
			var x = marge_gauche + colonne * (taille_brique.x + espacement_x) + taille_brique.x / 2.0
			var y = marge_haut + ligne * (taille_brique.y + espacement_y) + taille_brique.y / 2.0
